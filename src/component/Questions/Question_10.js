...
// return choce from localstroge
    useEffect(()=>{ const answer_saved=localStorage.getItem("q10Answer")
    ...
    function choice(number){
        setselected(number);
        localStorage.setItem("q10Answer",number)

    }

    const [question_number,setquestion_number]=useState(10)
    
    function next_question(){
        if (selected){move_to_nextquestion("/student/question-End")}
    ...
    function last_question(){
                if (question_number==10){move_to_nextquestion("/student/question-9")
    ...
    return(
        <div className="Question">
            ...
            <article >
                <h3 className="the_qustion">10-useState in react is used to ?</h3>
                <ul>
                    <li onClick={()=>choice(1)} className={selected===1?"active":""}>call APIs</li>
                    <li onClick={()=>choice(2)} className={selected===2?"active":""}>create components</li>
                    <li onClick={()=>choice(3)} className={selected===3?"active":""}>store and update values</li>
                    <li onClick={()=>choice(4)} className={selected===4?"active":""}>import files</li>
