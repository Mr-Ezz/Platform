...
// return choce from localstroge
    useEffect(()=>{ const answer_saved=localStorage.getItem("q8Answer")
    ...
    function choice(number){
        setselected(number);
        localStorage.setItem("q8Answer",number)

    }

    const [question_number,setquestion_number]=useState(8)
    
    function next_question(){
        if (selected){move_to_nextquestion("/student/question-9")}
    ...
    function last_question(){
                if (question_number==8){move_to_nextquestion("/student/question-7")
    ...
    return(
        <div className="Question">
            ...
            <article >
                <h3 className="the_qustion">8- Common react file extension is:?</h3>
                <ul>
                    <li onClick={()=>choice(1)} className={selected===1?"active":""}>.html</li>
                    <li onClick={()=>choice(2)} className={selected===2?"active":""}> .Css</li>
                    <li onClick={()=>choice(3)} className={selected===3?"active":""}> .js </li>
                    <li onClick={()=>choice(4)} className={selected===4?"active":""}>.jsx</li>
